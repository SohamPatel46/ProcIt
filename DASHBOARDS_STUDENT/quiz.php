		...
		$(document).ready(function(){
			...
			DetectRTC.load(function() {                    
				if(DetectRTC.isWebsiteHasWebcamPermissions){
				    document.getElementById("full-view").disabled = false; 
				}else{
				    document.getElementById("full-view").disabled = true; 
				    er2.innerHTML="Alert !! Give camera permission"	
				}
				//s1.innerHTML=DetectRTC.isWebsiteHasMicrophonePermissions;				
			});   
...
<body>
			...
			function start_timer(){
				...
				var x = setInterval(function() {
					...
					if(y!=screen_height && screen_height!=0){
						document.getElementById("er2").innerHTML = "Unfair Means !! Your quiz ends here";	
						er2.style.background="red";					
						setTimeout(function(){ 
	...
	<div style="overflow:hidden;position:fixed;background-color: #bfbfbf;top:0;width:100%;">
		...
		<p style="margin-left:50%;margin-top:7%;border-radius:10px;position:fixed;top:0;padding: 5px; text-align: center; font-size:30px;color: black"  id="er2"></p>
	</div>
	<input id="submit_expire" name="expire" style="margin-top:15%;margin-left:45%"  class="btn btn-outline-primary" type="submit" onClick="mySubmit_expire();" value="Submit Quiz">
		    ...
		    <div class="row">
		        <div class="" style="width: 100%;">
		            <div class="border">
						...
						<form id="myform_submit" class="needs-validation" novalidate="" action="../response.php" method="post">
						...
						<?php
							...
							?>
		                <div class="question bg-white p-3 border-bottom" style="height: 500px; margin-top:10px;width:125%;">
	...
	<script>
		function mySubmit(){
			document.getElementById('hiddenfield').value = localStorage.getItem("change_screen");
			$("#myform_submit").validate();
			document.getElementById('myform_submit').submit();  
			
		}
		function mySubmit_expire(){
			document.getElementById('hiddenfield').value = localStorage.getItem("change_screen");
			//$("#myform_submit").validate();
			document.getElementById('myform_submit').submit();  
	...
	<script>
		document.body.addEventListener("keyup", function(event) {
			if(!(event.keyCode>=48 && event.keyCode<=57) && !(event.keyCode>=65 && event.keyCode<=90) && (event.keyCode!=8) && (event.keyCode!=32) && (event.keyCode!=37) && (event.keyCode!=39))
			{
				counter+=1;
				er1.innerHTML="You tried to change screen : "+counter+" Times";	
				er1.style.background="red";
				localStorage.setItem("change_screen", counter);
			}
			else{
				er1.innerHTML="";
